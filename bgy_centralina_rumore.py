...
FIRMA_ACUSTICA_MODELLI = {
    ...
    'B752': {'base_db': 86.0, 'tipo': 'Boeing 757 Cargo (Forte impatto)'},
...
def calculate_noise(dist_orizzontale_km, quota_metri, modello_icao):
    ...
    rumore = firma['base_db'] - (20 * math.log10(dist_3d))
    if dist_3d > 1.5: rumore -= (dist_3d * 1.2)
...
def main():
        ...
        for centrale in centraline:
            ...
            for p in punti:
                try:
                    ...
                    if d < dist_min: dist_min = d; punto_critico = p
                except: continue
            if dist_min <= 5.0 and punto_critico:
                quota = float(punto_critico['altitude']) if punto_critico['altitude'] else 300.0
                noise_db = calculate_noise(dist_min, quota, 'DEFAULT')
                results.append({
                    'timestamp': punto_critico['timestamp'].split('T')[-1][:8], 'callsign': callsign, 'modello': 'DEFAULT',
                    'descrizione_aereo': 'Velivolo Standard', 'centralina': centrale['nome'], 'distanza_km': round(dist_min, 3),
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        if results:
            ...
            print(f"[RUMORE] ✅ Generato report acustico.")
        else:
            writer = csv.writer(f)
            writer.writerow(['timestamp', 'callsign', 'modello', 'descrizione_aereo', 'centralina', 'distanza_km', 'altitudine_m', 'rumore_stimato_db', 'valutazione'])
