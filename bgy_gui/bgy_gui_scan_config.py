"""
bgy_gui_scan_config.py - Tab configurazione scansioni e orari.
"""
...
class ScanConfigTab:
    """Tab configurazione scansioni e orari."""
    ...
    def _create_widgets(self):
        ...
        # Orario invio mail automatica
        frame4 = ttk.LabelFrame(main_frame, text="📧 Orario invio mail automatica", padding=10)
        ...
        self.report_time_entry.insert(0, str(self.config_data.get("daily_report_time", "06:15")))
        ttk.Label(frame4, text="La mail di stato viene inviata automaticamente a quest'ora ogni giorno", 
                  font=("Helvetica", 8), foreground="#7f8c8d").pack(anchor="w")
        
        # NUOVO: Orario caricamento su GitHub
        frame5 = ttk.LabelFrame(main_frame, text="🐙 Orario caricamento report su GitHub", padding=10)
        frame5.pack(fill="x", pady=5)
        ttk.Label(frame5, text="Orario (HH:MM):").pack(anchor="w")
        self.github_time_entry = ttk.Entry(frame5, width=20)
        self.github_time_entry.pack(anchor="w", pady=5)
        self.github_time_entry.insert(0, str(self.config_data.get("github_upload_time", "06:30")))
        ttk.Label(frame5, text="I report giornaliero e notturno vengono caricati su GitHub a quest'ora", 
                  font=("Helvetica", 8), foreground="#7f8c8d").pack(anchor="w")
    ...
    def save_config(self):
        try:
            ...
            github_time = self.github_time_entry.get().strip()
            ...
            self.config_data["github_upload_time"] = github_time
