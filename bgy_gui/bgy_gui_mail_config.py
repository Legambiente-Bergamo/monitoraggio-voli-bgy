"""
bgy_gui_mail_config.py - Tab configurazione destinatari email.
"""
...
class MailConfigTab:
    ...
    def __init__(self, parent, app):
        self.parent = parent
        self.app = app
        self.config_mail = config_manager.get_mail_config()
    ...
    def _create_widgets(self):
        ...
        # Popola lista
        for recipient in self.config_mail.get("recipients_daily", []):
            self.daily_listbox.insert(tk.END, recipient)
        ...
        ttk.Button(add_frame, text="➕ Aggiungi", 
                   command=lambda: self.add_recipient("daily", self.daily_entry, self.daily_listbox),
        ...
        for recipient in self.config_mail.get("recipients_monthly", []):
            self.monthly_listbox.insert(tk.END, recipient)
        ...
        ttk.Button(add_frame2, text="➕ Aggiungi", 
                   command=lambda: self.add_recipient("monthly", self.monthly_entry, self.monthly_listbox),
                   width=12).pack(side="left", padx=2)
        ...
        for recipient in self.config_mail.get("recipients_yearly", []):
            self.yearly_listbox.insert(tk.END, recipient)
        ...
        ttk.Button(add_frame3, text="➕ Aggiungi", 
                   command=lambda: self.add_recipient("yearly", self.yearly_entry, self.yearly_listbox),
                   width=12).pack(side="left", padx=2)
    ...
    def add_recipient(self, key, entry, listbox):
    ...
    def remove_recipient(self, listbox):
        """Rimuove il destinatario selezionato."""
        selection = listbox.curselection()
        if selection:
            listbox.delete(selection[0])
    
    def get_list_items(self, listbox):
        """Restituisce tutti gli elementi di una Listbox."""
        return [listbox.get(i) for i in range(listbox.size())]
    
    def save_config(self):
        try:
            daily = self.get_list_items(self.daily_listbox)
            monthly = self.get_list_items(self.monthly_listbox)
            yearly = self.get_list_items(self.yearly_listbox)
            
            self.config_mail["recipients_daily"] = daily
            self.config_mail["recipients_monthly"] = monthly
            self.config_mail["recipients_yearly"] = yearly
            
            if config_manager.save_mail_config(self.config_mail):
